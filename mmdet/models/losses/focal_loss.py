...
def sigmoid_focal_loss(pred,
    ...
    # pred torch.Size([200, 80])
    loss = _sigmoid_focal_loss(pred.contiguous(), target, gamma, alpha, None,
                               'none')
    # loss after sigmoid_focal_loss() torch.Size([200, 80])
    if weight is not None:
        if weight.shape != loss.shape:
            if weight.size(0) == loss.size(0):
                # For most cases, weight is of shape (num_priors, ),
        ...
        assert weight.ndim == loss.ndim
    loss = weight_reduce_loss(loss, weight, reduction, avg_factor)
    # loss after weight_reduce_loss() torch.Size([]) scalar
    return loss
...
class FocalLoss(nn.Module):
    ...
    def forward(self,
        ...
        if self.use_sigmoid:
            ...
            # TODO QueryInst的weight全是1，avg_factor却只计算了正样本
            if torch.cuda.is_available() and pred.is_cuda:
